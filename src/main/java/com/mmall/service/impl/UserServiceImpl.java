...
public class UserServiceImpl implements IUserService {
    ...
    // 效验用户名和邮箱是否已存在
    // 已存在返回error的ServerResponse, 不存在返回success的ServerResponse
    public ServerResponse<String> checkValid(String str, String type) {
        if (org.apache.commons.lang3.StringUtils.isNoneBlank(type)) {
            // isNoneBlank认为"  "是空的，返回false
            // 开始效验
    ...
    public ServerResponse<String> selectQuestion(String username) {
        ...
        if (org.apache.commons.lang3.StringUtils.isNoneBlank(question)) {
            return ServerResponse.createBySuccess(question);       // 放置到data属性
        }
    ...
    public ServerResponse<String> resetPassword(String passwordOld, String passwordNew, User user) {
        // 防止横向越权，要效验一下这个用户的旧密码，一定要指定是这个用户
    ...
    public ServerResponse<User> updateInformation(User user) {
        // username不能被更新
        // email也要进行一个效验，效验新的email是不是已经存在, 并且存在的email如果相同的话，不能是我们当前这个用户的
        int resultCount = userMapper.checkEmailByUserId(user.getEmail(), user.getId());
        if (resultCount > 0) {
            return ServerResponse.createByErrorMessage("email已被使用, 请尝试更换email");
        }
